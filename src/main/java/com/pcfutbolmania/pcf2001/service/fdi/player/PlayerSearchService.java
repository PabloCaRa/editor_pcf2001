...
public class PlayerSearchService {
	...
	public List<Player> searchPlayers(PlayerFilter filter, Map<Integer, Player> players) {
		List<Player> playersFiltered = players.values().stream().filter(player -> StringUtils
				.containsIgnoreCase(player.getName(), filter.getName())
				&& StringUtils.containsIgnoreCase(player.getShortName(), filter.getShortName())
				&& player.getBirthCountry() == (filter.getBirthCountry() != null ? filter.getBirthCountry().intValue()
						: player.getBirthCountry())
				&& player.getNationality() == (filter.getNationality() != null ? filter.getNationality().intValue()
				...
				&& (player.getParameters().getAggressiveness() >= filter.getAggressivenessMin().intValue()
						&& player.getParameters().getAggressiveness() <= filter.getAggressivenessMax().intValue())
